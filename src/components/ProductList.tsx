...
const LightboxGallery = ({ image, onClose }: { image: string, onClose: () => void }) => {
    ...
    return (
        <div className="fixed inset-0 z-[10000] bg-black/95 flex items-center justify-center overflow-hidden" onWheel={handleWheel}>
            ...
            <div className={`transition-transform duration-100 ease-out w-full h-full flex items-center justify-center ${isDragging ? 'cursor-grabbing' : scale > 1 ? 'cursor-grab' : 'cursor-zoom-in'}`} onMouseDown={handleMouseDown} onMouseMove={handleMouseMove} onMouseUp={handleMouseUp} onMouseLeave={handleMouseUp}>
                <img ref={imgRef} src={image} className="max-w-full max-h-full object-contain select-none" style={{ transform: `translate(${position.x}px, ${position.y}px) scale(${scale})` }} draggable={false} alt="Zoom Preview" onClick={(e) => { if(scale===1) { e.stopPropagation(); handleZoom(1); } }} />
            </div>
        </div>
    );
};

// --- MODAL DETAIL (FINAL FIX: SCROLL, LAYOUT VIDEO & PADDING) ---
    ...
    return (
        <>
            ...
            <div className="fixed inset-0 z-[9999] flex items-end md:items-center justify-center p-0 md:p-4">
                ...
                {/* MODAL: Lebar 98% (w-[98vw]) dan Grid dibagi 3:2 (col-span-3 untuk Media, col-span-2 untuk Teks) */}
                <div className="relative w-full h-[95vh] md:h-[95vh] md:w-[98vw] md:max-w-none bg-white dark:bg-[#0f172a] rounded-t-2xl md:rounded-3xl overflow-hidden shadow-2xl flex flex-col lg:grid lg:grid-cols-5 border-t md:border border-gray-200 dark:border-white/10 animate-slide-up" onClick={(e) => e.stopPropagation()}>
                    ...
                    {/* MEDIA SECTION (KIRI - 3 KOLOM) */}
                    <div className={`relative bg-black w-full lg:col-span-3 transition-all duration-300 group ${isExpanded ? 'h-0 opacity-0 lg:h-full lg:opacity-100' : 'h-[40vh] lg:h-full'}`}>
                        <div ref={sliderRef} className="w-full h-full flex overflow-x-auto snap-x snap-mandatory hide-scrollbar scroll-smooth">
                            {mediaItems.length > 0 ? mediaItems.map((item, index) => (
                                <div key={index} className="min-w-full w-full h-full snap-center flex items-center justify-center bg-black relative">
                                    ...
                                    <div className="relative z-10 w-full h-full flex items-center justify-center p-0 md:p-4 lg:p-10">
                                        {item.type === 'video' ? (
                                            /* Video: Fit ukuran aslinya (aspect-video) dan di tengah */
                                            <div className="w-full max-w-5xl aspect-video shadow-2xl bg-black border border-white/10 rounded-xl overflow-hidden">
                                                <iframe src={item.url} className="w-full h-full" frameBorder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share; fullscreen" allowFullScreen></iframe>
                                            </div>
                                        ) : (
                                            <div className="relative w-full h-full group/img cursor-pointer flex items-center justify-center" onClick={() => setLightboxImage(item.url)}>
                                                <img src={item.url} className="max-w-full max-h-full object-contain" alt={p.name} />
                        ...
                        {mediaItems.length > 1 && (
                            <>
                                <button onClick={(e) => { e.stopPropagation(); slide('left'); }} className="absolute left-2 md:left-4 top-1/2 -translate-y-1/2 z-20 w-10 h-10 md:w-12 md:h-12 bg-black/30 hover:bg-black/70 text-white rounded-full flex items-center justify-center backdrop-blur-sm border border-white/10 shadow-lg transition-all hover:scale-110 active:scale-95">❮</button>
                                <button onClick={(e) => { e.stopPropagation(); slide('right'); }} className="absolute right-2 md:right-4 top-1/2 -translate-y-1/2 z-20 w-10 h-10 md:w-12 md:h-12 bg-black/30 hover:bg-black/70 text-white rounded-full flex items-center justify-center backdrop-blur-sm border border-white/10 shadow-lg transition-all hover:scale-110 active:scale-95">❯</button>
                            </>
                        )}
                    </div>

                    {/* CONTENT SECTION (KANAN - 2 KOLOM) */}
                    <div className={`flex flex-col bg-white dark:bg-[#0f172a] relative z-20 overflow-hidden transition-all duration-300 ${isExpanded ? 'h-full lg:col-span-2' : 'flex-1 lg:h-full lg:col-span-2'}`}>
                        ...
                        {/* ================================================================================= */}
                        {/* ⚡ SCROLL AREA FIXED: PADDING BOTTOM BESAR (pb-60) AGAR TEKS TIDAK KEPOTONG ⚡ */}
                        {/* ================================================================================= */}
                        <div className="flex-1 min-h-0 px-6 py-6 overflow-y-auto custom-scrollbar bg-white dark:bg-[#0f172a]">
                            <div className="pb-60"> {/* PADDING BAWAH EKSTRA BESAR (240px) */}
                                <h2 className="text-2xl md:text-3xl font-black text-slate-900 dark:text-white mb-2 leading-tight">{p.name}</h2>
                                ...
                                <div className="prose prose-sm md:prose-base prose-slate dark:prose-invert text-gray-700 dark:text-gray-200 leading-relaxed w-full max-w-full break-words whitespace-normal [&_img]:max-w-full [&_img]:h-auto [&_iframe]:max-w-full [&_table]:block [&_table]:overflow-x-auto">
                                    <div dangerouslySetInnerHTML={{ __html: p.description || '...' }} />
                                </div>
                            </div>
                        </div>
                        
                        <div className="p-4 border-t border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-[#1e293b] flex items-center gap-3 relative z-30">
                            <div>
